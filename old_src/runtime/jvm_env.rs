...
use gc::allocator_local::AllocatorLocal;
use gc::space::Space;
use gc::tlab::initialize_tlab;
...
impl JvmEnv {
    pub fn new(jre_opt: Option<String>, cp_opt: Option<String>) -> Self {
        let space = Arc::new(Space::new(1024 * 1024 * 1024));
        let allocator_local = AllocatorLocal::new(space.clone());
        initialize_tlab(allocator_local);
